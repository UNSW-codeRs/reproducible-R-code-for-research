...
# We want only 100 records from the data.frame, if library `dplyr` is available we can use the `slice_head` function, otherwise we can use simply `head`

if (require(dplyr)) {
  ss.data <- original.data %>% 
    slice_head(n=100)
} else {
  ss.data <- head(original.data,n=100)
}
...
target.file <- sprintf("%s/modified-data.csv", my.temp.dir)
